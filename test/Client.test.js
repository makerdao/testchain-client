...
const _create = async options => {
  ...
  if (options.scenario_id) {
    ...
    return client.sequenceEvents(id, [Event.OK, Event.READY, Event.ACTIVE]);
  }
};
...
const _restart = id => {
  client.restart(id);
  return client.sequenceEvents(id, [Event.READY]);
};
...
test(
  'client will create a normal chain instance',
  async () => {
    await client.init();
...
test('client will restart a stopped chain', async () => {
  ...
  expect(Object.keys(eventData)).toEqual([Event.READY]);
...
test(
  ...
  async () => {
    ...
    const {
      ready: { id: id2 }
    } = await _create({ ...options, clean_on_stop: true });
...
test('client will restore a snapshot', async () => {
  ...
  } = await _takeSnapshot(id, 'SNAPSHOT');
