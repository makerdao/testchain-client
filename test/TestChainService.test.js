...
test.only('clear chain', async () => {
  ...
  const id1 = await service.createChainInstance(opts1);
  await service.joinChain(id1);
  await service.joinChain(id2);
  const id2 = await service.createChainInstance(opts1);
  console.log('chain list before', service.getChainList());
  await service.clearChains();
  console.log('chain list after', service.getChainList());
});
...
test('will join only one chain at a time', async () => {
  const chainId1 = await service.createChainInstance({
    http_port: 8545,
    accounts: 3,
    block_mine_time: 0,
